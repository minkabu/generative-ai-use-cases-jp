...
import InputPdfChatContent from '../components/InputPdfChatContent';
...
const usePdfChatPageState = create<StateType>((set) => {
  return {
    content: '',
    inputSystemContext: '',
    saveSystemContext: '',
...
const PdfChatPage: React.FC = () => {
  ...
  const title = useMemo(() => {
    if (chatId) {
      return getConversationTitle(chatId) || 'PDFチャット';
    } else {
      return 'PDFチャット';
    }
